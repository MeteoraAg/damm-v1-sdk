...
mod tests {
    ...
    use common::dynamic_vault::pda::derive_lp_mint_key;
    use prog_dynamic_vault::state::Vault;
    use solana_rpc_client::rpc_client::RpcClient;
    use solana_sdk::signature::Keypair;

    #[test]
    fn test_vault_lp_mint() {
        let rpc_client = RpcClient::new(Cluster::Devnet.url().to_owned());

        let program = anchor_client::Client::new(
            anchor_client::Cluster::Custom(rpc_client.url(), rpc_client.url()),
            Rc::new(Keypair::new()),
        )
        .program(prog_dynamic_vault::ID)
        .unwrap();

        let pools = program.accounts::<Vault>(vec![]).unwrap();

        let mut pool_key_with_lp_mint = vec![];

        for (key, state) in pools {
            let pool_lp_mint = derive_lp_mint_key(key);
            if pool_lp_mint != state.lp_mint {
                println!("{}", key);
                pool_key_with_lp_mint.push((key, state.lp_mint));
            }
        }

        let open = "lazy_static! {
    static ref VAULT_WITH_NON_PDA_BASED_LP_MINT: HashMap<Pubkey, Pubkey> = HashMap::from_iter([";
        let close = "])}";

        let inner = pool_key_with_lp_mint
            .into_iter()
            .map(|(pool_key, lp_mint_key)| {
                format!(
                    "(pubkey!(\"{}\"), pubkey!(\"{}\")),",
                    pool_key.to_string(),
                    lp_mint_key.to_string()
                )
            });

        let full = format!("{} {}{}", open, inner.collect::<String>(), close);
        println!("{}", full);
    }
}
